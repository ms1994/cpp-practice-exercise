...
int main() {
  ...
  } else {//camino k/2 step y me regreso
    vector<vector<int>> new_ans(h, vector<int>(w, 0));
    for (int it = 0; it < k / 2; it++) {
      for (int i = 0; i < h; i++) {
        for (int j = 0; j < w; j++) {
          new_ans[i][j] = (int) 1e9;//evalua el minimo valor en las cuatro dirreciones del punto en el
          //rectangulo, calcula todos los puntos a la vez por cada step
          //utiliza el punto anterior para saber cual es su min camino que tomo ese punto
          if (i > 0) {
